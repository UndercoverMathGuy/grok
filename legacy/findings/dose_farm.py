"""Suite B — cross-mask dose-response farm (claim 3's causal-breadth gap).

Until now every causal surgery lived on ONE mask (data_seed 0, base
seed27058). This farm replicates the energy dose-response on three new base
runs, each on a DIFFERENT mask, plus the missing x1.50 point on seed27058.

Bases (all p=113, natural, spectra-logged, epoch-0 checkpoints):
  runs/seed1/seed21245          mask ds=1
  runs/seed2/seed51376          mask ds=2
  runs/p-113/seed2034/seed33428 mask ds=2034
Target frequency per base: chosen by the same pre-registered rule as
surgery.py — the most median-energy frequency (|z| minimal) that is NOT in
the e3000 top-8 menu, NOT in the committee, and NOT harmonically (2k, 3k)
or additively (i+-j) related to any committee member.

Doses per new base: x1.00 (control rerun), x1.05, x1.10, x1.20, x1.50,
x2.25 energy on the target's W_E Fourier block at epoch 0. 12k epochs.

PRE-REGISTERED PREDICTIONS (from the seed27058 curve: 0/0/1477/4459/19735
at 1.0/1.05/1.10/1.2/2.25):
  P-B1: per base, final |coeff| of the target is monotone in dose.
  P-B2: adoption threshold lies in (1.05, 1.20] energy on every mask
        (at x1.20 the target is committee-adopted; at x1.05 it is not).
  P-B3: the x1.00 control reproduces the base run's natural committee.
  P-B4: every final committee is drawn from the run's own e3000 top-8 menu
        (menu closure); bystander recomposition is allowed (chaos).
Primary outcome variable: final target |coeff| (detector-free), with
adoption = final |coeff| > 10x the background median.
"""
import sys
from pathlib import Path
import numpy as np

ROOT = Path("/Users/ruhaanrajadhyaksha/projects/grok")
sys.path.insert(0, str(ROOT)); sys.path.insert(0, str(ROOT / "findings"))
import mlx.core as mx
from grok.config import Config
from grok.model import Transformer
from grok.fourier import Fourier
from grok.train import train
from mask_lottery import committee_from_coeffs

SCRATCH = Path(__file__).parent
P = 113
nf = P // 2
fourier = Fourier(P)
EPOCHS = 12000

BASES = ["seed1/seed21245", "seed2/seed51376", "p-113/seed2034/seed33428"]
DOSES = [1.00, 1.05, 1.10, 1.20, 1.50, 2.25]

def fold(x):
    x %= P
    return min(x, P - x)

def freq_energy(F):
    E = (F ** 2).sum(0)
    return E[1::2][:nf] + E[2::2][:nf]

def pick_target(comm, menu, z):
    excl = set(menu) | set(comm)
    for k in comm:
        excl |= {fold(2 * k), fold(3 * k)}
        for j in comm:
            if j != k:
                excl |= {fold(k + j), fold(k - j)}
    cands = [k for k in range(1, nf + 1) if k not in excl]
    return cands[int(np.argmin([abs(z[k - 1]) for k in cands]))]

def run_arm(base_rel, dose, model, W_E, F, target, cfg_path, tag):
    name = f"dose_{int(round(dose * 100)):03d}"
    run_dir = ROOT / "runs" / "dosefarm" / tag / name
    if (run_dir / "spectra.npz").exists():
        print(f"skip {tag}/{name} (exists)", flush=True)
    else:
        Fm = F.copy()
        s = np.sqrt(dose)
        Fm[:, 2 * target - 1] *= s
        Fm[:, 2 * target] *= s
        W = W_E.copy()
        W[:, :P] = Fm @ fourier.basis
        model.embed["W_E"] = mx.array(W.astype(np.float32))
        ck = SCRATCH / f"dosefarm_{tag}_{name}.safetensors"
        model.save_weights(str(ck))
        c2 = Config.load(cfg_path)
        c2.num_epochs = EPOCHS
        c2.save_every = 4000
        print(f"\n=== {tag} {name} (target {target} x{dose}) ===", flush=True)
        train(c2, run_dir, init_from=ck, spectra_every=100, log_every=4000)
    z = np.load(run_dir / "spectra.npz")
    final = committee_from_coeffs(z["coeffs"][-1])
    gi = int(np.argmax(z["test_acc"] >= 0.99))
    ge = int(z["epochs"][gi]) if z["test_acc"][gi] >= 0.99 else -1
    tr = np.abs(z["coeffs"][:, target - 1])
    bg = float(np.median(np.abs(z["coeffs"][-1])))
    print(f"RESULT {tag} {name}: grok@{ge} committee {final} "
          f"target f{target} peak {tr.max():.0f} final {tr[-1]:.0f} "
          f"(bg median {bg:.1f}) acc {z['test_acc'][-1]:.4f}", flush=True)

for base_rel in BASES:
    d = ROOT / "runs" / base_rel
    cfg = Config.load(d / "config.json")
    assert cfg.p == P
    z = np.load(d / "spectra.npz")
    comm = committee_from_coeffs(z["coeffs"][-1])
    i3k = int(np.argmin(np.abs(z["epochs"] - 3000)))
    menu = (np.argsort(np.abs(z["coeffs"][i3k]))[::-1][:8] + 1).tolist()
    model = Transformer(cfg)
    model.load_weights(str(d / "checkpoints" / "epoch_00000.safetensors"))
    W_E = np.array(model.W_E, dtype=np.float64)
    F = W_E[:, :P] @ fourier.basis.T
    energy = freq_energy(F)
    zsc = (energy - energy.mean()) / energy.std()
    target = pick_target(comm, menu, zsc)
    tag = d.name
    print(f"\n##### base {base_rel}: committee {comm}, menu {menu}, "
          f"target {target} (z {zsc[target-1]:+.2f}) #####", flush=True)
    print("predictions: P-B1 monotone; P-B2 adopt at >=1.20, not at <=1.05; "
          "P-B3 control reproduces; P-B4 menu closure", flush=True)
    for dose in DOSES:
        model.load_weights(str(d / "checkpoints" / "epoch_00000.safetensors"))
        run_arm(base_rel, dose, model, W_E, F, target, d / "config.json", tag)

# missing x1.50 point on the original curve (base seed27058, target 7)
d = ROOT / "runs" / "og_seed0" / "seed27058"
cfg = Config.load(d / "config.json")
model = Transformer(cfg)
model.load_weights(str(d / "checkpoints" / "epoch_00000.safetensors"))
W_E = np.array(model.W_E, dtype=np.float64)
F = W_E[:, :P] @ fourier.basis.T
print("\n##### base og_seed0/seed27058 gap-fill: target 7 x1.50 #####",
      flush=True)
run_arm("og_seed0/seed27058", 1.50, model, W_E, F, 7, d / "config.json",
        "seed27058")
print("\nDOSE FARM DONE", flush=True)
