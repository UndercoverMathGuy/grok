"""Tied-embedding variant: V is shared by both operands (like the
transformer's shared W_E). Does tying create mask favoritism?"""
import sys
...
def run_toy(seed, is_train, is_test):
    ...
    state = (V, U, mx.zeros_like(V), mx.zeros_like(U),
             mx.zeros_like(V), mx.zeros_like(U), mx.array(0.0))

    def step(V, U, mV, mU, sV, sU, t):
        H = V @ Osum.T                       # tied: same V for a and b
        A = H * H
        R = A.T @ U.T - y
        gU = R.T @ A.T / n
        Bm = (U.T @ R.T) * (2 * H) / n
        ...
        t = t + 1
        out = []
        for P_, g, m, s in ((V, gV, mV, sV), (U, gU, mU, sU)):
            m = B1 * m + (1 - B1) * g
            s = B2 * s + (1 - B2) * g * g
            out.append((P_ - LR * ((m / (1 - B1**t)) /
                       (mx.sqrt(s / (1 - B2**t)) + EPS) + WD * P_), m, s))
        (V, mV, sV), (U, mU, sU) = out
        return V, U, mV, mU, sV, sU, t
    ...
    for s in range(MAXS):
        ...
        if s % 1000 == 999:
            mx.eval(*state)
            Vn, Un = (np.array(t2, np.float64) for t2 in state[:2])
            logits = ((Vn[:, a] + Vn[:, b]) ** 2).T @ Un.T
            te = (logits[is_test].argmax(1) == all_lab[is_test]).mean()
            if te >= 0.999 and s >= 3999:
...
print("\n=== tied-toy within-mask consistency ===")
for ds in (0, 1, 2):
...
print("\n=== tied-toy vs TRANSFORMER ===")
same, cross = [], []
for ds_t in (0, 1, 2):
    for ds_f in (0, 1, 2):
        v = [len(x & y) for x in toy[ds_t] for y in tf[ds_f]]
        ch = np.mean([len(x) * len(y) / 56.0 for x in toy[ds_t] for y in tf[ds_f]])
        o = np.mean(v)
